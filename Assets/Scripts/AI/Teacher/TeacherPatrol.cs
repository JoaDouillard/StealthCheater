...
public class TeacherPatrol : MonoBehaviour
{
    ...
    public PatrolPoint GetNextRandomPoint(Vector3 fromPosition)
    {
        ...
        // FIX: Fallback SANS contrainte de distance minimale (juste sur NavMesh)
        Debug.LogWarning($"[TeacherPatrol] ⚠️ Aucun point valide après {maxAttempts} tentatives, fallback SANS contrainte de distance");

        // Réessayer 10 fois SANS contrainte de distance
        for (int fallbackAttempt = 0; fallbackAttempt < 10; fallbackAttempt++)
        {
            ...
            {
                lastRandomPoint = fallbackNavMesh;
                return new PatrolPoint
                {
        ...
        // FIX: Dernier recours - Chercher n'importe où sur le NavMesh proche
        Debug.LogError("[TeacherPatrol] ❌ Fallback échoué! Utilisation du dernier recours (point proche sur NavMesh)");

        // Chercher un point aléatoire proche sur NavMesh (rayon 10m)
        for (int i = 0; i < 20; i++)
        {
            Vector3 randomDirection = Random.insideUnitSphere * 10f;
            randomDirection.y = 0; // Garder sur le plan horizontal
            Vector3 targetPoint = currentPosition + randomDirection;

            if (IsPointOnNavMesh(targetPoint, out Vector3 nearbyNavMesh))
            {
                lastRandomPoint = nearbyNavMesh;
                return new PatrolPoint
                {
                    position = nearbyNavMesh,
                    type = PointType.RandomNavMesh,
                    interestTransform = null,
                    windowIndex = -1
                };
        ...
        // ABSOLU dernier recours - Rester sur place (évite crash)
        Debug.LogError("[TeacherPatrol] ❌❌ IMPOSSIBLE de trouver un point NavMesh! Reste sur place.");
        return new PatrolPoint
        {
